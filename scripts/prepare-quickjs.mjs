...
const target = resolve(workspace, ".cache", "quickjs");
const expected = lock.quickjs.commit;
...
if (!existsSync(resolve(target, ".git"))) {
  ...
  git(["remote", "add", "origin", lock.quickjs.repository], target);
}
...
if (actual !== expected) {
  throw new Error(`QuickJS pin mismatch: expected ${expected}, got ${actual}`);
}

console.log(`QuickJS ${actual}`);
