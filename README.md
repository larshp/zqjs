...
- hoisted named functions, parameters, calls, returns, recursion, and mutable lexical
  closures;
- 73 verified QuickJS-aligned opcode IDs, constant/local slots, patched jumps, a
...
`node --expose-gc`, self-tests the test262 metadata parser, and then runs thirteen pinned
test262 language and JSON cases through the transpiled zqjs engine. Positive and negative
